...
def _is_valid_profile(data: dict) -> bool:
    """business/sector/products는 비어있지 않은 문자열, competitors는 비어있지 않은
    문자열들의 리스트인지 검증한다. 캐시에서 읽은 값(예: 예전 문자열 형식의 competitors)
    검증에도 재사용한다."""
...
def generate_all_profiles(
    ...
    종목에 대한 불필요한 반복 호출을 줄인다. 캐시에 없는 종목들은 스레드풀(max_workers)로
    동시에 생성해 전체 실행 시간을 줄인다."""
